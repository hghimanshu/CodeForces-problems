...
class Solution:
    ...
    def largestSubtree(self, root, array):
        if root:
            left_depth = self.calculateDepthatANode(root.left, 0)
            right_depth = self.calculateDepthatANode(root.right, 0)

            array.append(max(left_depth, right_depth))
            self.largestSubtree(root.left ,array)
            self.largestSubtree(root.right, array)
        return max(array)
